...
static PyObject* vl6180_initialise(PyObject *self, PyObject* args) {
	int device;
	int agent;
	if(!PyArg_ParseTuple(args, "i", &device)) return NULL;
	agent = vl6180_initialise_c(device);
	return Py_BuildValue("i", agent);
}

static PyObject* get_distance(PyObject *self, PyObject* args) {
	int handle, result;
	if(!PyArg_ParseTuple(args, "i", &handle)) return NULL;
	result = get_distance_c(handle);
	return Py_BuildValue("i", result);
}

static struct PyMethodDef VlMethods[] = {
	{"init", vl6180_initialise, METH_VARARGS, "Initialization"},
	{"get_distance", get_distance, METH_VARARGS, "Get distance"},	
	{NULL, NULL, 0, NULL}
};

void initvl6180(void) {
	Py_InitModule("vl6180", VlMethods);
}
...
int vl6180_initialise_c(int device){
	int handle=-1;
	char buf[15];
